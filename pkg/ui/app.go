...
func NewAppUI(app *tview.Application, client *api.Client) tview.Primitive {
	...
	// Create VM list with initial items
	...
	// Auto-refresh VM status colors every 5 seconds
	go func() {
		...
		processBatch := func(start, end int) {
			...
			for i := 0; i < end-start && i < len(vmsAll); i++ {
				select {
				case result := <-results:
			...
			if len(updates) > 0 {
				app.QueueUpdateDraw(func() {
					for _, update := range updates {
						text := fmt.Sprintf("%d - %s", vmsAll[update.idx].ID, update.name)
						if update.isActive {
		...
		for range ticker.C {
			...
			batchSize := 3
			for i := 0; i < len(vmsAll); i += batchSize {
				end := i + batchSize
				if end > len(vmsAll) {
					end = len(vmsAll)
				}
				...
				if i+batchSize < len(vmsAll) {
					time.Sleep(500 * time.Millisecond)
				}
			}
		}
	}()
	// Ensure VM details panel has a border
