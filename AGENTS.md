...
- **`internal/cli/`** - Non-interactive CLI subcommands (nodes, guests, tasks, storage)
  ...
  - `community_scripts.go` - Community Scripts search/show/plan/install/deploy CLI commands, gated by `plugins.enabled: ["community-scripts"]`
...
- **Community Scripts core package**: Shared Community Scripts metadata/search/install logic lives in `internal/plugins/communityscripts`. The TUI plugin wraps it from `internal/ui/plugins/communityscripts`; CLI code should depend on the core package, not UI plugin code. For unattended CLI provisioning, use `community-scripts plan` before `community-scripts deploy --yes --set var_name=value`; only allowlisted Community Scripts `var_*` overrides should be passed through. Include `var_container_storage` and `var_template_storage` for unattended LXC installs so upstream scripts do not open a storage picker. When both storage overrides are provided, pvetui seeds `/usr/local/community-scripts/default.vars` before invoking upstream scripts and must restore or remove that file afterward; keep this behavior intact because upstream first-run defaults can prompt even with storage env vars set. `--yes` should keep upstream default-mode and host-update prompts suppressed, and external installer success should clear target profile cluster inventory cache before detecting created guests.
...
- **Degraded cluster recovery**: During node/storage outages, cluster-wide guest discovery can hang even when node-local APIs still work. Prefer `pvetui guests list --node <node> --node-local` for emergency inventory and direct lifecycle targeting (`--node <node> --type qemu|lxc`) when the guest's location is already known.
