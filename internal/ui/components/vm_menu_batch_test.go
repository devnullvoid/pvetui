...
func TestIsEligibleForBatchAction(t *testing.T) {
	...
	templateQemu := &api.VM{Status: api.VMStatusStopped, Type: api.VMTypeQemu, Template: true}

	require.True(t, isEligibleForBatchAction(stoppedQemu, vmActionStart))
	require.False(t, isEligibleForBatchAction(templateQemu, vmActionStart))
	...
	require.False(t, isEligibleForBatchAction(templateQemu, vmActionShutdown))
	...
	require.False(t, isEligibleForBatchAction(templateQemu, vmActionReset))
